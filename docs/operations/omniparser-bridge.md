...
`omniparser-bridge` HTTP service so the wsl1 endpoint never lands
on argv.
...
3. Set `OMNIPARSER_BRIDGE_URL=http://127.0.0.1:18090` in the
   ...
   with `OMNIPARSER_BRIDGE_SECRET` (must match the wsl1-side env).
...
The wsl1-side bridge listens on `127.0.0.1:8090`; the runx tunnel
...
| `OMNIPARSER_BRIDGE_SECRET` | yes | 32-byte HMAC key shared with the wsl1 bridge. |
...
- The bridge runs as `nonroot:nonroot` in a distroless static image.
  The wsl1 systemd unit should set `NoNewPrivileges=true`,

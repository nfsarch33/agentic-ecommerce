...
The current API can require a static bearer token through
`ECOMMERCE_API_TOKEN`. v0.9.0 JWT/RBAC work must preserve that boundary while
moving user sessions to short-lived JWT access tokens, refresh-token rotation,
and router-level role checks for `admin`, `operator`, and `viewer`.

Required JWT/RBAC deployment controls:
...
- Use Redis-backed counters for multi-instance deployments.

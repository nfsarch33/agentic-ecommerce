...
- `host-a/node-a` as the merge-blocking `primary-testing` contract for backend
...
- `secondary-testing` stays standby until `host-b`, `host-b-travel`, `node-b`, and
  `node-b-travel` all satisfy controller SSH, trust, cleanup, and resource-health
  gates.

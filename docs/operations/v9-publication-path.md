...
`v9.0.0` semver tag. The primary-testing lane on `node-a/host-a` is the only
...
```bash
# On primary-testing (node-a/host-a)
...
- `node-b`, `host-b`, `host-b-travel` timeout from the controller -- non-blocking.

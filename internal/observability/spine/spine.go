...
func MetricInventory() []MetricContract {
	return []MetricContract{
		...
		{Name: "ec_workerpool_size", Kind: Gauge, Owner: "workerpool", Description: "Adaptive worker pool target size.", Labels: []LabelContract{{Name: "pool", MaxCardinality: 16}}, DashboardFields: []string{"workerpool_size"}},
		{Name: "ec_workerpool_resize_total", Kind: Counter, Owner: "workerpool", Description: "Adaptive worker pool resize events.", Labels: []LabelContract{{Name: "pool", MaxCardinality: 16}, {Name: "direction", MaxCardinality: 2, Values: []string{"grow", "shrink"}}}, DashboardFields: []string{"workerpool_resize_total"}},
		...
		{Name: "ec_resource_guard_alerts_total", Kind: Counter, Owner: "runtimeobs", Description: "Resource guard alerts by bounded signal and severity.", Labels: []LabelContract{{Name: "signal", MaxCardinality: 3, Values: []string{"heap", "goroutine", "sentrux_desktop"}}, {Name: "severity", MaxCardinality: 2, Values: []string{"warning", "critical"}}}, DashboardFields: []string{"resource_guard_alerts_total"}},
		{Name: "ec_sentrux_desktop_process_count", Kind: Gauge, Owner: "runtimeobs", Description: "Observed Sentrux desktop process count from runx/cursor-tools probes.", DashboardFields: []string{"sentrux_desktop_process_count"}},
		{
...
func SnapshotFromCapsule(c evomap.Capsule) DashboardSnapshot {
	...
	return DashboardSnapshot{
		...
		Fields: []KPIField{
			...
			{Name: "workerpool_size", Value: float64(k.WorkerpoolSize), Unit: "count"},
			{Name: "workerpool_resize_total", Value: float64(k.WorkerpoolResizeTotal), Unit: "count"},
			...
			{Name: "resource_guard_alerts_total", Value: float64(k.ResourceGuardAlertsTotal), Unit: "count"},
			{Name: "sentrux_desktop_process_count", Value: float64(k.SentruxDesktopProcessCount), Unit: "count"},

...
- A pluggable EvoMap feed must capture per-minute KPIs as both an
  ...
  watch.
- Heavy upstreams (OmniParser, MiniMax) must run on the node-a fleet
...
on argv), never the node-a IP literal.
...
on the MacBook and live on the node-a fleet host. They are reached
via signed bridges:
...
- **`pushgateway` runx tunnel** -- the existing v2.10.0 forward
  resolves node-a -> 127.0.0.1:19091 on the MacBook. The bridge is
  pure HTTPS basic-auth; no app code needs to know the node-a
  endpoint.
...
heavy compute stays on node-a and the MacBook stays a thin client.
...
- `omniparser-bridge` is reusable for any future fleet-resident
  upstream (planned v3.1: rerank model on node-b).

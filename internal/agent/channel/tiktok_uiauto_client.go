...
// poster on the wsl1 side. It is the client-side facade; the real
...
func NewTikTokUIAutoClient(logger *slog.Logger, cfg TikTokUIAutoClientConfig) (*TikTokUIAutoClient, error) {
	...
	if strings.TrimSpace(cfg.BridgeURL) == "" {
		return nil, fmt.Errorf("%w: BridgeURL required (runx alias for omniparser-bridge wsl1)", ErrUIAutoUnconfigured)
	}
